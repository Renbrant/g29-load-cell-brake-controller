/*
  ...
  - v2.010: Atualizados os valores de calibração da célula de carga para maior precisão:
           Offset = -598930
           Fator de Calibração = -20442.525391.
           Manutenção da alta velocidade de leitura do sistema.
*/
...
// **Valores de calibração ajustados**
const float CALIBRATION_FACTOR = -20442.525391; // Atualizado
const long OFFSET = -598930;                    // Atualizado
float pressure = 0;
...
void setup() {
  ...
  scale.set_offset(OFFSET);                // Aplica offset calibrado
  scale.set_scale(CALIBRATION_FACTOR);     // Aplica fator de calibração calibrado
...
float readPressure() {
  float measuredPressure = scale.get_units(10); // Média de 10 leituras para suavização
