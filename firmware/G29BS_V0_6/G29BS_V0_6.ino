/*
   Versão: v0.6 - Sistema de Leitura de Pressão e Saída de Tensão para Simulador de Freio
   ...
   Alterações nesta versão (v0.6):
   1. Remoção da limitação de 3.3V para refletir a alimentação do Arduino Pro Mini a 3.3V.
   2. Configuração da taxa de amostragem para 80 SPS no HX711.
   3. Escala máxima ajustada para 30 kg.
   4. Potenciômetros configurados para ajuste de sensibilidade (5 a 30 kg) e curva de resposta.
   5. Gráfico no display OLED com rótulos reposicionados para melhor visualização.
...
void loop() {
  ...
  float adjustedMaxPressure = map(potSensitivity, 0, 1023, 5, 30); // Ajusta de 5 a 30 kg como pressão máxima
  ...
  uint16_t dacOutput = (uint16_t)(adjustedPressure * 4095); 
  dacOutput = constrain(dacOutput, 0, 4095); // Limita o valor para 3.3V máximo
