...
struct Config {
  float maxVoltage;
  float curveFactor;
  float curveShape; // Novo parâmetro: formato da curva
};
...
float curveShape = 0.0;           // Formato da curva inicial (0 = linear)
int lastEncoderPosition = 0;
bool buttonPressed = false;
int configMode = 0; // 0 = Vmax, 1 = Curva, 2 = Formato da Curva
...
void loop() {
  ...
  if (currentButtonPressed && !buttonPressed) {
    configMode = (configMode + 1) % 3; // Alterna entre Vmax, Curva e Formato
    saveSettings(); // Salva as configurações na flash
    Serial.println(configMode == 0 ? "Ajustando Vmax" : configMode == 1 ? "Ajustando Curva" : "Ajustando Formato");
  }
  ...
  if (newEncoderPosition != lastEncoderPosition) {
    ...
    if (configMode == 0) {
      ...
      // Ajusta o formato da curva
      curveShape += delta * 0.1; // Incrementa ou decrementa em passos de 0.1
      curveShape = constrain(curveShape, -1.0, 1.0);
      Serial.print("Formato da curva ajustado para: ");
...
void updateDAC() {
  ...
  // Aplica o formato da curva (exponencial ou inversa) mantendo os extremos fixos
  float adjustedPressure;
  if (curveShape > 0) { // Curva convexa (barriguda para cima)
    adjustedPressure = pow(normalizedPressure, 1.0 / (1.0 + curveShape));
  } else if (curveShape < 0) { // Curva côncava (barriguda para baixo)
    adjustedPressure = 1.0 - pow(1.0 - normalizedPressure, 1.0 / (1.0 - curveShape));
  } else { // Curva linear
    adjustedPressure = normalizedPressure;
  }

  // Ajusta o fator de curva
  adjustedPressure *= (1 + curveFactor);
  adjustedPressure = constrain(adjustedPressure, 0.0, 1.0);

  // Calcula a tensão de saída com base em Vmax e pressão ajustada
  float outputVoltage = MIN_VOLTAGE + adjustedPressure * (maxVoltage - MIN_VOLTAGE);

  // Atualiza o DAC apenas se a pressão mudou significativamente
  if (abs(outputVoltage - previousPressure) > 0.01) { // Evita atualizações redundantes
    uint16_t dacOutput = (uint16_t)((outputVoltage / 3.3) * 4095); // Converte para 12 bits
    dac.setVoltage(dacOutput, false);
    previousPressure = outputVoltage; // Atualiza o valor anterior
  }
}
...
void drawGraph() {
  ...
  for (int x = 0; x < GRAPH_WIDTH; x++) {
    ...
    if (curveShape > 0) { // Curva convexa
      curveY = pow(rawPos, 1.0 / (1.0 + curveShape));
    } else if (curveShape < 0) { // Curva côncava
      curveY = 1.0 - pow(1.0 - rawPos, 1.0 / (1.0 - curveShape));
    } else { // Curva linear
      curveY = rawPos;
    }

    // Ajusta o fator de curva
    curveY *= (1 + curveFactor);
