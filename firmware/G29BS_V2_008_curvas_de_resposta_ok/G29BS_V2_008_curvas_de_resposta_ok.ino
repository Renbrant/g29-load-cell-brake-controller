/*
  ...
  - v2.008: Reorganizada a configuração do formato da curva com dois passos:
           1. Inclinação da curva.
           2. Nível de curvatura (exponencial ou logarítmico).
           Configurações de Vmax e Vmin vêm após a definição da curva.
           Correção da funcionalidade de inclinação e ajustes integrados ao formato da curva.
*/
...
struct Config {
  ...
  float curvatureLevel; // Nível de curvatura (exponencial ou logarítmico)
};
...
float curvatureLevel = 0.0;       // Nível de curvatura inicial (linear)
...
void handleButtonPress() {
  ...
  if (currentButtonState && !buttonPressed) {
    if (!showConfigMenu) {
      ...
      configMode = (configMode + 1) % 4; // Alterna entre Inclinação, Curvatura, Vmax e Vmin
    }
...
void handleEncoderRotation() {
  if (showConfigMenu) { // Permite ajustes apenas quando o menu está ativo
    ...
    if (newEncoderPosition != lastEncoderPosition) {
      ...
      } else if (configMode == 1) { // Ajusta o Nível de Curvatura
        curvatureLevel += delta * 0.1;
        curvatureLevel = constrain(curvatureLevel, -2.0, 2.0);
      } else if (configMode == 2) { // Ajusta o Vmax
        maxVoltage += delta * 0.05;
        maxVoltage = constrain(maxVoltage, 2.5, 3.3);
      } else if (configMode == 3) { // Ajusta o Vmin
...
void updateDAC() {
  ...
  // Aplica curvatura (exponencial ou logarítmica)
  if (curvatureLevel > 0) { // Exponencial
    adjustedPressure = pow(adjustedPressure, 1.0 + curvatureLevel);
  } else if (curvatureLevel < 0) { // Logarítmico
    adjustedPressure = pow(adjustedPressure, 1.0 / (1.0 - curvatureLevel));
  }
...
void updateDisplay() {
  ...
  if (showConfigMenu) { // Exibe o menu de configurações, se ativo
    ...
    display.print(configMode == 1 ? "> Curv: " : "  Curv: ");
    display.println(curvatureLevel, 2);
    display.print(configMode == 2 ? "> Vmax: " : "  Vmax: ");
    display.println(maxVoltage, 2);
    display.print(configMode == 3 ? "> Vmin: " : "  Vmin: ");
...
void drawGraph() {
  for (int x = 0; x < GRAPH_WIDTH; x++) {
    ...
    if (curvatureLevel > 0) { // Exponencial
      curveY = pow(curveY, 1.0 + curvatureLevel);
    } else if (curvatureLevel < 0) { // Logarítmico
      curveY = pow(curveY, 1.0 / (1.0 - curvatureLevel));
    }
...
void saveSettings() {
  Config config = {maxVoltage, minVoltage, curveFactor, curvatureLevel};
...
void loadSettings() {
  ...
  curvatureLevel = config.curvatureLevel;
  ...
  if (curvatureLevel < -2.0 || curvatureLevel > 2.0) curvatureLevel = 0.0;
  ...
  Serial.println(curvatureLevel, 2);
}
