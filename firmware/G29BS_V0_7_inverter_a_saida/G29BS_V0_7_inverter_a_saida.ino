/*
   Versão: v0.6 Modificada - Sistema de Leitura de Pressão e Saída de Tensão para Simulador de Freio
   ...
   uma saída de tensão de 3.3V (mínimo de pressão) a 0V (máximo de pressão) no DAC.

   Alterações nesta versão:
   1. Inversão da lógica da saída do MCP4725 para refletir maior tensão com menos pressão e vice-versa.
   2. Ajuste da escala máxima do sistema para 50 kg.
...
void loop() {
  ...
  // Inversão da saída para que a tensão máxima seja com menos pressão e vice-versa
  uint16_t dacOutput = (uint16_t)((1.0 - adjustedPressure) * 4095); 
